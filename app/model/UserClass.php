<?php
class UserClass implements Nette\Security\IAuthenticator
{
  private $database;
  private $passwords;
  ...
  public function authenticate(array $credentials): Nette\Security\IIdentity
  {
    ...
    $role = $this->database->table('role')
      ->where('id_role', $row->role)->fetch();

    return new Nette\Security\Identity($row->id_user, ['first_name' => $row->first_name, 'last_name' => $row->last_name, 'role' => $role->name, 'email' => $row->email]);
  }
